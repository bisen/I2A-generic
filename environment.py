...
def recover_images(image_onehots):
    image_onehots = image_out.reshape(BATCH_SIZE, 186, 160, 5)
    for img_onehot in img_onehots:
...
class EnvModel:
    ...
    def train_episode(self, feed_dict):
        return self.session.run([self.loss, self.optimizer, self.image], feed_dict=feed_dict)
...
if __name__ == '__main__':
    ...
    for it, states, actions, rewards, next_states, is_done in next_batch(10):
        ...
        loss, _, image = env_model.train_episode(feed_dict={
