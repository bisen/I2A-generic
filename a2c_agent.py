...
VALUE_FACTOR = 1.0
RENDER = False
# MINIBATCH_SIZE: The number of games to be played before updating the weights
MINIBATCH_SIZE = 8
# PER_TIMESTEP_REWARD: Continous reward that is given at each timestep
...
class A2C:
    def __init__(self, game, sess=None):
        self.game = game
        self.num_actions = 3
        ## one observation consists of 2 consecutive frames (cropped) stacked on top of each other
        self.state_size = [STACK_SIZE, 186, 160, 3]
        with tf.variable_scope('a2c'):
            ...
            # self.common is the trunk output of which will then be fed into actor and the critic
            ...
            if sess:
                # if a session is provided, use that
                self.session = sess
    ...
    # Testing method for viewing how the agent performs by just playing a bunch of games
    def play(self):
        self.load_last_checkpoint()
        while True:
            st = self.game.reset()
    ...
    ## common is the trunk output of which will then be fed into actor and the critic
    def common(self):
    ...
    def train_episode(self):
        # variables for storing cumulative actions, rewards and states
        c_actions = []
        c_rewards = []
        c_states = []
